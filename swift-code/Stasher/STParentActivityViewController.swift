...
//
//  STParentActivityViewController.swift
//  Stasher
//
...
//
import UIKit
protocol ParentActivityDelegate: NSObjectProtocol {
    func parentActivityCellDidSelect(withDict dict: [AnyHashable: Any])
}
class STParentActivityViewController: UIViewController {
    var httpReq: BOABHttpReq?

    weak var delegate: ParentActivityDelegate?
    ...
    var globalActivityButton: UIButton?
...
// MARK: ----- ActivityTableView Delegtes
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        var simpleTableIdentifier: String = "STParentGlobalActivityCustomCell"
        var cell: STParentGlobalActivityCustomCell? = tableView.dequeueReusableCell(withIdentifier: simpleTableIdentifier)
        if cell == nil {
            cell = (Bundle.main.loadNibNamed(NSStringFromClass(STParentGlobalActivityCustomCell.self), owner: self, options: [])[0] as? STParentGlobalActivityCustomCell)
        }
        if globalActivityArray.count > indexPath.row {
            if myActivityArray[indexPath.row] {
                ...
                cell?.labelActivityTitle?.text = "\(globalActivityArray[indexPath.row][kParamKeyUsername])"
                cell?.labelActivityTime?.textColor = UIColor.stasherTextFieldPlaceHolder()
                cell?.labelActivityTime?.font = UIFont.fontGothamRoundedMedium(withSize: 12.0)
                var dateFormat = DateFormatter()
                dateFormat.dateFormat = "yyyy-MM-dd HH:mm:ss"
                ...
                var theDate2: String = dateFormat2.string(from: Date())
                var dt: Date? = dateFormat.date(fromString: theDate2)
                var minutes: uint? = dt?.timeIntervalSince(theDate) / 60
                if minutes < 1 {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if delegate?.responds(to: #selector(self.parentActivityCellDidSelectWithDict)) {
            delegate?.parentActivityCellDidSelect(withDict: myActivityArray[indexPath.row])
        }
    }
}