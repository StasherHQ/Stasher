//
//  STParentActivityViewController.m
//  Stasher
//
...
@interface STParentActivityViewController ()

@end

@implementation STParentActivityViewController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    static NSString *simpleTableIdentifier = @"STParentGlobalActivityCustomCell";
    STParentGlobalActivityCustomCell *cell = [tableView dequeueReusableCellWithIdentifier:simpleTableIdentifier];
    
    if (cell == nil) {
        cell = [[[NSBundle mainBundle] loadNibNamed:NSStringFromClass([STParentGlobalActivityCustomCell class]) owner:self options:0] objectAtIndex:0];
    }
    if ([self.globalActivityArray count] > indexPath.row) {
        if ([self.myActivityArray objectAtIndex:indexPath.row]) {
            [cell.labelDescription setAttributedText:[[NSAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@",[[self.globalActivityArray objectAtIndex:indexPath.row] objectForKey:@"description"]]
                                                                                               attributes:@{
                                                                                                            NSForegroundColorAttributeName:[UIColor stasherTextColor],
            ...
            [cell.labelActivityTitle setText:[NSString stringWithFormat:@"%@",[[self.globalActivityArray objectAtIndex:indexPath.row] objectForKey:kParamKeyUsername]]];
            
            [cell.labelActivityTime setTextColor:[UIColor stasherTextFieldPlaceHolderColor]];
            [cell.labelActivityTime setFont:[UIFont FontGothamRoundedMediumWithSize:12.0f]];
            NSDateFormatter *dateFormat = [[NSDateFormatter alloc] init];
            [dateFormat setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
            ...
            NSString *theDate2 = [dateFormat2 stringFromDate:[NSDate date]];
            NSDate *dt = [dateFormat dateFromString:theDate2];
            uint minutes = [dt timeIntervalSinceDate:theDate] / 60;
            if (minutes < 1) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self.delegate respondsToSelector:@selector(parentActivityCellDidSelectWithDict:)]) {
        [self.delegate parentActivityCellDidSelectWithDict:[self.myActivityArray objectAtIndex:indexPath.row]];
    }
}
